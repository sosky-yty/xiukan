...
import com.example.sosky.xiukan.Configurations;
import com.example.sosky.xiukan.Properties;
...
public class StartActivity extends AppCompatActivity {
    ...
    public void readProperties() {
        Request request = new Request.Builder()
                ...
                .build();
        XiuKan.HTTP_CLIENT.newCall(request).enqueue(new Callback() {
            @Override
            ...
            public void onResponse(Call call, Response response) throws IOException {
                final Properties properties = XiuKan.parseJson(Properties.class, response.body().string());
                if (properties != null) {
    ...
    public void handleProperties(Properties properties) {
        XiuKan.DATA_SOURCES.clear();
        XiuKan.DATA_SOURCES.addAll(properties.getDataSources());

        XiuKan.hostReplacements.clear();
        for (DataSource source : XiuKan.DATA_SOURCES) {
            try {
                String host = new URI(source.getLink()).getHost();
                for (String h : source.legacies) {
                    XiuKan.hostReplacements.put(h, host);
                }
    ...
    private void checkPermissions() {
                    ...
                    .withListener(new PermissionListener() {
                        ...
                        public void onPermissionDenied(PermissionDeniedResponse response) {
                            new AlertDialog.Builder(StartActivity.this)
                                    .setTitle("权限申请")
                                    .setCancelable(false)
                                    .setMessage("XiuKan 需要储存空间权限，储存用户配置。请您允许。")
        ...
        File config = new File(XiuKan.getStorageDir(), "configurations.json");
        if (oldConfig.exists()) {
            oldConfig.renameTo(config);
        }

        File noMedia = new File(XiuKan.getStorageDir(), ".nomedia");
        try {
        ...
        XiuKan.CONFIGURATIONS = Configurations.load(config);
